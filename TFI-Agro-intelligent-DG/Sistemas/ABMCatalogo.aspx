<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="ABMCatalogo.aspx.cs" Inherits="TFI_Agro_intelligent_DG.Sistemas.ABMCatalogo" Async="true" %>

<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <h1>ABM Packs</h1>
    <asp:Label ID ="hola" runat="server" Text="<%$ Resources:Resource.language, Hello %>"></asp:Label>
    <table id="PacksTable" class="table table-striped table-bordered" style="width:100%;">
        <thead>
            <tr>
                <th>Id</th>
                <th>Descripción</th>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainScriptsContent" runat="server">
    <script>
        ...
        function CargarPacks(data) {


            return fetch('https://localhost:44325/Packs/GetPack', {
                method: 'POST',
                ...
                .then(res => res.json())
                .then(res => { window.localStorage.setItem('packs', res) })
                .then(res => { console.log(res) })
                .then(res => ProcesarCargarPacks(res));
        }

        const ProcesarCargarPacks = () => {
            var packs = JSON.parse(window.localStorage.getItem('packs'));
            console.log("packs:", packs);
            $('#PacksTable >tbody').html('');
            $.each(productos, (index, value) => {
                $('#PacksTable >tbody').append(`<tr><td>${value.id}</td><td>${value.producto}</td><td>${value.cantidad}</td><td><span style="cursor:pointer;" onclick="addProduct('${value.producto}'); RefrescarTabla();">+</span> / <span style="cursor:pointer;" onclick="removeProduct('${value.producto}'); RefrescarTabla();">-</span></td><td>${value.precio}</td><td>${value.cantidad * value.precio}</td></tr><tr><td colspan=6 align=center><span style="cursor:pointer;" onclick="GuardarPedido()">Finalizar Pedido</span></td></tr>`);
            });
        }
